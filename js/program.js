document.addEventListener("DOMContentLoaded", () => {
  ...
  openBtn.addEventListener("click", () => {
    ...
    editMode = false;
    editId = null;
    document.getElementById("createProgramLabel").innerText = "Create Your Workout";
    modal.show();
  });
  ...
  async function loadMyPrograms() {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/programs/user/${username}`);
  ...
  function editProgram(id, programs) {
    ...
    editMode = true;
    editId = id;
    document.getElementById("createProgramLabel").innerText = "Edit Workout";
...
document.getElementById("viewWorkouts").addEventListener("click", async () => {
  ...
  if (list.style.display === "none" || !list.style.display) {
    ...
    document.getElementById("viewWorkouts").innerText = "Hide My Workouts";
  } 
